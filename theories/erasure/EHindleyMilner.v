(** * EHindleyMilner: a type-inference forward map λ□ → λ□ᵀ, verified as a
    ...
    - SCOPE OF THE ALGORITHM: inference is best-effort; it does not claim
      principality or completeness (neither holds on all of λ□: □ residue,
      polymorphic recursion, unrepresentable types).  Wherever HM cannot assign
      a principal ML type it falls back to [TAny] (⊤ / success-typing style),
      keeping [infer] TOTAL.
    ...
    2. CROSS-CONSTANT SCHEME PROPAGATION.  Constants are processed in dependency
       ...
       fresh variables ([instantiate]).  Constants not yet in the table — e.g.
       forward/mutual references across the ordering — degrade gracefully to a
    ...
    3. DATATYPE-DRIVEN TERM INFERENCE from GIVEN signatures.  [infer] takes a
       ...
       tForce), or a unification/fuel failure.  [empty_sigs] assigns [TAny] to
       every datatype node.
    ...
    4. TOWARD λ□ᵀ WELL-FORMEDNESS.  The MetaRocq install imported here exposes no
       ...
       λ□ᵀ program" lemma is stated as a RESIDUAL.  What IS
    ...
    1. [PAst.v] [PAst_to_ExAst] promotes an untyped program into the typed world
       via [Ok (infer empty_sigs env)] on its [Untyped] arm.

    2. [Pipeline.v] runs [infer] on untyped input for a typed backend and then
       the [typed_transform_pipeline].  The section lemma licenses that gate: it
       guarantees the promoted environment erases back to the original untyped
       one, so the untyped correctness results transfer.

    3. RESIDUAL: once a [check_wf_typed_program]-style predicate is in scope (it
       is absent from the imported MetaRocq), prove that [infer D Σ] satisfies it
       — i.e. that the inferred [box_type] annotations are coherent (well-scoped
       tvars, arities matching, schemes instantiable).  This is the ExAst-only
       content the section law cannot transport.  Refining [infer_oib] from [D]
       (see note above) is a prerequisite for the datatype-body part.
    ------------------------------------------------------------------------ *)
