...
From Peregrine Require CatCryptBackend.
...
Definition validate_ast_type (c : config) (p : PAst) : result' unit :=
  ...
  | C _ | Wasm _ | OCaml _ | CakeML _ | CatCrypt _ | Eval _ => Ok tt
  | AST c =>
...
Definition needs_typed (c : config) : bool :=
  match c.(backend_opts) with
  | Rust _ | Elm _ => true
  | C _ | Wasm _ | OCaml _ | CakeML _ | CatCrypt _ | Eval _ => false
  | AST c =>
    match c.(ast_type) with
    | LambdaBoxTyped => true
    | _ => false
...
| CatCryptProgram : string -> extracted_program
...
Definition run_backend (c : config) (f : string) (p : PAst) : extraction_result :=
  ...
  | CatCrypt opts =>
    p' <- PAst_to_EAst p;;
    res <- CatCryptBackend.extract_catcrypt
      const_remaps
      custom_attr
      opts
      f
      p';;
    Ok (CatCryptProgram res)
